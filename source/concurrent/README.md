...
**注意，`close` 函数不是一个清理操作，而是一个控制操作，在确定这个 channel 不会在发送数据时调用。**
...
**注意上面的代码会报 `fatal error: all goroutines are asleep - deadlock!`。这个是死锁的错误，因为 `range` 不等到信道关闭是不
会结束读取的。也就是如果 `channel` 没有数据了，那么 `range` 就会阻塞当前 `goroutine`, 直到信道关闭，所以导致了死锁**。
...
**对于通道中的同一个元素值来说，发送操作和接收操作之间也是互斥的。例如，虽然会出现，正在被复制进通道但还未复制完成的元素值，但是这
时它绝不会被想接收它的一方看到和取走**。
