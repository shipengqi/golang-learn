...
## 常量
前面我们已经了解了变量。这里学习常量。
`const`声明常量，注意常量的**底层数据类型只能是基础类型（布尔型、数值型和字符串型）**：
```go
const 常量名字 类型 = 表达式
```
...
```
常量表达式的值在**编译期计算**。因此常量表达式中，函数必须是内置函数。如`len()`, `cap()`, `unsafe.Sizeof()`**。

批量声明的常量，除了第一个常量，剩下的常量都可以省略初始化表达式，如果省略初始化表达式则使用前面常量的初始化表达式写法，
例如：
```go
const (
    a = 1
