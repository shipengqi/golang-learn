...
`main.main()` 表明了这个 goroutine 包装的 go 函数就是命令源码文件中的那个`main`函数，也就是说这里的 goroutine 正是**主 goroutine**。
...
`defer` 关键字对应的 `runtime.deferproc` 会将延迟调用函数与调用方所在 goroutine 进行关联。所以当程序发生崩溃时只会调用**当前 goroutine** 的延迟调用函数。
