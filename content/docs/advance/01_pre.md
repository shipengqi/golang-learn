...
用户态地址空间：每个用户进程都有自己的虚拟地址空间。操作系统通过虚拟内存管理（如分页和分段机制）**为每个进程分配独立的地址空间**。这些地址空间在用户态下是独立的，进程之间不能直接访问彼此的地址空间。

内核态地址空间：**内核有自己的地址空间**，通常是一个全局的地址空间。内核态的代码可以访问所有进程的地址空间，但用户态的代码不能直接访问内核地址空间。
{{< /callout >}}
