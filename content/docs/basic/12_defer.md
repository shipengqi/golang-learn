...
- `runtime.deferreturn` **负责在函数调用结束时执行所有的延迟调用**，会插入在调用 `defer` 的函数的末尾；
...
- 从调度器的 `defer` 缓存池 `sched.deferpool` 中取出结构体并将该结构体追加到当前 goroutine 的缓存池中；
...
**`runtime.deferreturn` 会从 goroutine 的 `_defer` 链表中取出最前面的 `runtime._defer` 结构体**，并**调用 `runtime.jmpdefer` 函数传入需要执行的函数和参数**：
...
`runtime.jmpdefer` 是一个用汇编语言实现的运行时函数，它的主要工作是**跳转到 `defer` 所在的代码段并在执行结束之后跳转回 `runtime.deferreturn`**。
...
- 函数的 `defer` 数量**少于或者等于 8 个**；
- 函数的 `defer` 关键字**不能在循环中执行**；
